...
#fileB = TFile('posterior-handmade-obs.root')
...
for key in keys:
    ...
    if 'RplusS' in key.GetName() and not ('T2' in filenameA): 
    ...
    hMethod.GetYaxis().SetRangeUser(0.01,30000000)
    hObserved.GetYaxis().SetRangeUser(0.01,30000000)	
    ...
    hratio = FabDraw(cGold,leg,hObserved,[hMethod],datamc='MC',lumi='X', title = '', LinearScale=False, fractionthing= 'observed/method')
